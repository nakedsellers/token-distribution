 ...
 */
contract NakedSale is Pausable {
    ...
    // The ratio of NAKED to Ether
    uint public rate;
    uint public constant LOW_RANGE_RATE = 2500;
    uint public constant HIGH_RANGE_RATE = 5000;

    // prevent certain functions from being recursively called
    bool private rentrancy_lock = false;

    // The token being sold
    NakedToken public tokenReward;
    ...
    /**
     * Constructor for a crowdsale of NakedToken tokens.
     *
     ...
     * @param rateNakedToEther              the conversion rate from NAKED to Ether
    ...
    function NakedSale(
        ...
        setRate(rateNakedToEther);
        tokenReward = NakedToken(addressOfTokenUsedAsReward);
    }
    ...
    function () payable whenNotPaused beforeDeadline afterStartTime saleNotClosed nonReentrant {
        ...
        // and NAKED have the same number of decimal places (18)
    ...
    /**
     * The owner can update the rate (NAKED to ETH).
     *
     * @param _rate  the new rate for converting NAKED to ETH
     */
     ...
     * are in units of wei and mini-NAKED. Every digit counts.
     *
     * @param _to               the recipient of the tokens
     * @param amountWei         the amount contributed in wei
     * @param amountMiniNaked   the amount of tokens transferred in mini-NAKED
     */
    function ownerAllocateTokens(address _to, uint amountWei, uint amountMiniNaked) external
            onlyOwner nonReentrant
    {
        if (!tokenReward.transferFrom(tokenReward.owner(), _to, amountMiniNaked)) {
            revert();
        }
    ...
    */
    ...
    /**
     * Given an amount in NAKED, this method returns the equivalent amount
     * in mini-NAKED.
     *
     * @param amount    an amount expressed in units of NAKED
     */
    function convertToMiniNaked(uint amount) internal constant returns (uint) {
